...
Screenshots from the owner's play session, 2026-08-23. His filenames, kept as he
named them, because the names are the report. Each one has an entry in
`docs/PENDING.md`.

| file | what it shows | entry |
|---|---|---|
...
| `daily_cartoonish2.png` | The same, second angle | "Daylight is flat and cartoonish" |
| `broken_hands_phone.png` | An NPC playing the phone idle with no phone: fingers curled round nothing, wrist reading as broken | "A phone idle with no phone" |
| `bald_people_wrong_hair_render.png` | Hair not drawing | "A phone idle with no phone" has the cast context; hair needs its own diagnosis |
| `side_walls_weird_texture.png` | Side walls, texture wrong | not yet diagnosed |
| `crash_2.png` | The WebGL context loss | "The periodic freeze" and the context-loss note |

Two he reported without a shot, both in `docs/PENDING.md`:

- The dialogue examples, quoted verbatim in "The dialogue does not answer the
  question" and "The brief is too thin, so the reply echoes it".
- "Fane's Counter", the place name he could not recall afterwards, in "Place
