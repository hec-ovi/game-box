 ...
 * run into, to see that what was timed is what was meant, `BLOCKS` how big a
 * town the street view builds (default 2), `WHOLE=1` every building whole at
 * every distance, and `SHELL` how far the shells reach (`all` for a shell on
 * every plot in the town, which is what a city cost before the skyline
 * carried the far field).
 *
 ...
 *   CHROME=/path/to/chrome node game/scene/tools/bench/measure.ts room 0 2 4
 */
...
try {
  ...
  const param = view === 'room' ? 'casters' : 'lights'
  ...
  const reach = process.env['SHELL'] ? `&shell=${process.env['SHELL']}` : ''
  for (const value of values.length ? values : ['16']) {
    console.log(await run(session, `${PAGE}?view=${view.replace('-gl', '')}&${param}=${value}${backend}${blocks}${look}${reach}`))
  }
  session.close()
} finally {
  browser.kill()
