...
| `Crowd.attend(...)` | `Attention` in [src/attention.ts](src/attention.ts) | a hold, taken the frame it is asked for: `face(x, y, z)` moves the point as the player moves, `release()` lets them carry on, `held` says whether the hold is still worth anything. A hold on somebody who has since gone home does nothing |
