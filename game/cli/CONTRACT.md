...
|---|---|---|
| `gb build` | `--theme --seed --blocks NxN --cells --density --storeys --exits --history --out` | a model writes every word of a city, so the sidecar has to be running on `GAME_BOX_URL`; a stage it will not write stops the build and nothing is written. `--storeys` is `@gb/forge`'s own ceiling unless you name one (24, 40 at most): over four the city raises a skyline of kit towers. `--history <file>` is a JSON object: `@gb/world`'s `Premise` plus `charters`, each a `CharterSchema`; it is taken as the narrator's answer to `writePremise`, so the forge gates it the way it gates a model's and the report says what was dropped |
| `gb extend <base>` | `--count --out` | `base` is a bundle. The growth is drawn with the base's own seed, `--count` is how many buildings to add (default 10) and the land decides how many fit |
...
- **A model writes every word of a city, and there is no way to ask for one written any other way.** `build` and `extend` go to `@gb/scribe` and nowhere else, so a model that will not answer stops the command, which says which stage and why and writes nothing. What is retried, dropped or settled rather than fatal is `@gb/scribe`'s to say: a side errand it cannot write is dropped and reported, a name it repeats is kept, and pay outside its band is settled.

## How to modify this blackbox safely

A new command is a new module, a parser in `src/args.ts`, a line in the switch and in the usage text. Keep `run(argv, io)` free of `process` and `console`. Who writes a city is `src/narrator.ts` alone: the model, and a history file in place of the story it would write. Opening a file for `inspect`, `check`, `pack` and `apply` is `src/open.ts`, which is where a refusal is put into words; `src/walk.ts` is the reachability walk `check` adds. What a city is pinned to lives in `src/pins.ts` alone; the size a pin is made against is the one `@gb/scene` hands `Dressing.building`, so it moves only when that does. The label a grown city and its pack are generated under is `GROWTH` in `src/extend.ts`, and `apply` reads it back off the pack. The cities the tests read are laid out rather than written (`tests/city.ts`: `@gb/forge`'s narrator-free plan, pinned and sealed the way a command seals one), because writing one is a model's and has no stand-in, so `build` and `extend` are exercised only as far as they get before the first question. `tests/fixtures/before-charters.json` is a `schemaVersion` 1 city kept as it was shared, the proof that an older file is opened and said to be older; `tests/fixtures/history.json` is a history with one charter the gate takes and one it drops. The `gb` script runs on `node --experimental-transform-types`, which is what `@gb/prefab` needs to load from source. Run `pnpm --filter @gb/cli test`.
