...
| 256 m | 563 ms | 425 MB | 373,729 | 318,129 | 11 | 10.95 ms | 51.42 ms |
| 128 m | 471 ms | 386 MB | 71,978 | 43,198 | 6 | 2.81 ms | 15.67 ms |

At the published 256 m the town holds a thirteenth of the geometry and submits a fourteenth of it, opens in a third of the time and in 42% of the memory. The skyline is 41,868 triangles of that, twelve a plot, and it does not move with the reach. Turning on the spot from the spawn at 256 m the four headings submit 36,156, 1,876, 197,674 and 318,129 triangles; with a shell on every plot the same four are 435,679, 1,876, 1,306,545 and 4,439,633. The rubbish is charged to the streets and is the same either way: 649,626 held, 508,186 in the draw.

**The frame is bought back on the walk.** A shell on every plot is built once at open, so a walk costs nothing; streaming pays for it a building at a time. One shipped-kit tower is 36 ms in the dressing and 18 ms copied into the batch, which is the worst frame in that table and is `STREAM_BUILDS` doing exactly one building. That ceiling is what a 23-storey shell costs to build, not what the streaming costs: the batch never grew on that walk and packed four times, none of them over half a millisecond. `node game/prefab/tools/bench-city.ts --blocks 20 [--shell 256|all]` is where the table comes from.
