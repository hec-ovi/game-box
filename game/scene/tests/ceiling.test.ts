...
function cost(root: THREE.Object3D, only: (name: string) => boolean = () => true): { meshes: number; triangles: number; materials: number } {
  ...
  root.traverse((child) => {
    ...
    if (mesh.isBatchedMesh) {
      const held = (mesh as unknown as { _geometryInfo: Array<{ count: number; active: boolean }> })._geometryInfo
      triangles += held.reduce((total, one) => total + (one.active ? one.count / 3 : 0), 0)
    } else {
...
describe('what a city costs', () => {
  ...
  it('keeps the triangles under the budget they were measured at', async () => {
    ...
    // measured: 26,914 for 142 buildings, ground, kerbs, paint and the mountain
    // ring. A greybox building is a box and a door slab, so this is the floor a
    // real kit is measured against, not a target
    expect(triangles).toBeLessThan(40_000)
    expect(triangles / world.plots().length).toBeLessThan(300)
  })
  ...
  it('still culls each building on its own, and lets one be taken out of the city', async () => {
    const world = await bigTown()
    const city = buildCity(world, new Greybox())
    const batches = city.root.children.filter((child) => (child as THREE.BatchedMesh).isBatchedMesh) as THREE.BatchedMesh[]

    expect(batches.length).toBeGreaterThan(0)
    // a merge would draw the whole town every frame; a batch throws away what
    // the frustum does not reach, building by building
    for (const batch of batches) expect(batch.perObjectFrustumCulled, batch.name).toBe(true)
