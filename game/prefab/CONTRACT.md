...
| `PrefabDressing.shell(plot, size, charter)` | the same | for every plot within `@gb/scene`'s `SHELL_RADIUS`; `lights` is never asked after it |
...
| `shell(plot, size, charter)` | `THREE.Object3D` | the same building from far off, in the same box: its walls alone on `Library.shell`, the same entrance, the same pictures on the same faces, no glass and no signs. It reads no street face, because nothing is seated on a shell. A plot the catalogue has no shape for gets its massing instead, on the same material: the box the plot occupies, painted with the wall its look wears, 26 triangles. It falls through to the `rest` only where no look in the pack claims the charter at all |
...
| `catalogue.look(plot, suits)` | a look's name, or undefined | the look this plot wears whatever its shape, for a plot the pack has no model at: the same rule `design` picks by, over the looks the charter's `suits` claim rather than over one bucket. Undefined only for an empty catalogue |
| `Library.walls(look)` | `{ wall, base }` layers, or undefined | which two layers of the strip that look is painted with, read off its own models. What a massing wears |
...
| `paneAt(facing)` | `{ reflected, through }` | what the pane gives back to the street and what it lets past at that facing, which is its alpha and its transmittance. 0.04 and 0.96 face on |
| `shownAt(level, lit)`, `litShare(lit, keys)` | a multiplier, and a share | how much of its picture one window shows at that hour, and how many windows of that kind have their lights on. The shader's own answers, without a GPU |
...
A plot the catalogue has no shape for is not an error: `building` hands it to the dressing behind, and `shell` draws its massing.
...
- `@gb/forge` contract, in the tests and the tools and nowhere in `src/`: `Forge.plan`, the arithmetic half of a city, is the town the shells, the windows and the cost are measured over. A shell and a window are read off a plot's footprint and its height, neither of which anybody writes, so nothing there needs a city that has been written. A seated fixture is not: how much signage a plot carries is what its kind of place decides, and a plan raises every building under the architecture's one word, so the fixtures are read over the pack's own street instead (`tools/town.ts`).
...
- **A pin this pack cannot honour falls back to the dressing behind.** Three
  ...
  this catalogue covers. All three draw a kit building near the player and this
  box's massing from far off, and none of them picks a different prefab. A kit
  building on a prefab street reads as a fallback; a quietly substituted model
  reads as the city the file describes, which is the failure worth being loud
  about.
...
- **After dark a room is lit while the city's lit share is above its key**, the same rule `@gb/kitbash` uses, so the same rooms come on in the same order every night and none of them flickers. A shopfront takes about a third of the key an office does, because a street of shops is lit and a street of offices is not.
- **A room reads through the glass at every hour, and that is measured.** Who has their lights on is a night question: the city's lit share is 0.03 at noon, which holds nine shopfronts in ten at `UNLIT`, so in daylight a window shows its picture whole and only after dark does a dark one go dark. On top of that a room burns at `ROOM.day` at noon against `ROOM.glow` after dark, because a shop keeps its lights on and the pane in front of it reflects the sky whatever the hour. Measured with `node tools/measure-glass.ts`, standing 3 m off a shopfront on the shipped pack, the pane's share of the pixel head on is **26 to 39% at noon and 23 to 37% at 21:00**, over a room worth 0.012 to 0.016 either way, so the glass reads the same at noon as it does at nine. Hold the room to the night's own reading instead and the pane is 93 to 96% of it at noon over a room worth 0.0004: a mirror with nothing behind it. Sixteen metres along the wall, where a pane is nearly edge on, it is 86 to 88% at every hour, and that is the reflection doing what glass does.
...
**The street this is read over is the pack itself.** How much signage a plot
carries and of what sort is what its kind of place decides, so a town where
every building stands under one word carries a nameplate and two door lamps and
says nothing about a blade on a flank, a box hung over the street or the lit box
on a station's doorstep. `tools/town.ts` crosses the two sides instead: every one
of the pack's 512 models on a plot cut to its own shape, pinned the way a world
file pins one, under the fourteen kinds of place `@gb/world` ships in turn, with
its door on each of the four walls and its doorstep moving along the front.

Measured with `node tools/measure-fixtures.ts` over that street: 512 buildings,
1,024 door lamps and 1,144 flat plates. Both columns come off one run: every
fixture is read against the face the model drew, once where the kit wrote it
against the plot and once where it was seated.

| | written | seated |
|---|---|---|
| lamp off 22 cm outside the drawn door frame | 0.035 to 4.876 m, 1.135 median | 0 |
| lamp head against the drawn head plus 15 cm | 0.199 to 0.800 m short | 0 |
| plate standing off the face under it | -0.120 to 0.080 m | 0.080 m, every one |
| plates in the same plane as that face | 204 of 1,144 | 0 |

The plate written on a fascia band is the one thing still not right: 109 of them
land on a band and 20 are a 0.758 m plate on a 0.600 m band, because the kit
...
Run it with `node tools/build-buildings.ts`. It needs `glb-buildings` beside the checkout, or `GLB_BUILDINGS` pointing at it. `node tools/measure-city.ts` then says what a planned town costs on the result.
...
- **A pin that cannot be honoured falls back; it never picks again.** Another pack's name, a model this copy no longer holds, or a shape the catalogue does not cover all hand the plot's whole building to the dressing behind. A kit building on a prefab street reads as a fallback, and a substituted prefab reads as the city the file describes, which is the failure worth being loud about.
...
- The catalogue covers every shape `@gb/world`'s `PLOT_BAND` cuts: frontage 3 to 6 cells by depth 5 to 8 by storeys 1 to 4, sixty-four shapes, eight looks in each, read in the door's frame through `plotShape`. A taller plot, a cell size that is not `METRICS.cellSize`, or a footprint outside that band has its whole building drawn by the dressing behind, which is why `@gb/kitbash` stays load-bearing; its shell is a massing drawn here.
- **A city's towers are the dressing behind, not a squashed prefab.** `@gb/forge` raises plots past the band so a city has a skyline, and there is no model in the pack for any of them: `design` answers nothing, `pin` writes nothing into the file, and `building` hands the plot on at its full height. A tower near the player is visibly a kit building on a street of prefabs, which is the honest answer, and drawing one shrunk into a four storey bucket would look exactly like the city the file describes. Measured on three 8 block seeds at density 1 and the brief's own default: every plot inside the band has a model, and every plot over it has none at every height up to `TALLEST_STOREYS`.

- **From far off a tower is its massing, on the pack's own wall.** Past `@gb/scene`'s `DETAIL_RADIUS` a building is a silhouette with lit windows in it, and both come out of the shell material over a box, so a plot the catalogue has no shape for wears one: four walls cut into a street-level band, the storeys over it and a metre of parapet, plus a flat roof, laid at the metres the producer laid its pictures at (12 m across a wall repeat, two 3.2 m floors down it) so the bays land at 3 m and the windows light in the same order as the street's. It is 26 triangles against the kit stack's 7,439, it carries the same five attributes as a pack shell so both are one buffer and one draw, and one geometry serves every plot of that shape and finish. Which wall it wears is the look the charter's `suits` picks, read off that look's own models rather than off a table.

  What it costs on screen: a tower keeps its size, its lit windows and its shape from across town, and loses the kit's setbacks, balconies and modelled entrance until the player is within `DETAIL_RADIUS`, where the kit's building stands as it always did. Measured on the metro 20 by 20 city at 24 storeys: 714 of 3,489 plots, 4.89M shell triangles down to 78,230, nine far draws down to two, and 195 ms of the open down to 55 to 97.
...
- Nothing on the outside of a building glows in daylight. The neon, the wall's own glow, the screens and the street in the glass are the night level times what is painted there, which is the same `CityNight` the kit's windows and lamps read, so one `setTime` moves the whole street. A room behind glass is the one exception and it is measured above: it burns at `ROOM.day` at noon, because a room carrying nothing but its own albedo comes to 0.0004 against the 0.0073 the pane over it reflects of the sky. By day the glass reflects the sky the app lights the city with.
...
Measured headless in Node with `node tools/measure-city.ts`, a planned 4 by 4 block town (170 plots, every one on the pack), the way `@gb/scene` batches it. No look in the pack claims the word the architecture raises a building under, so every plot of a plan draws from the whole of its bucket and the town comes out near the pack's own average; a written city picks inside the looks its charters claim.

| | walls | glass | shell |
|---|---|---|---|
| triangles a building | 216 | 19 | 216 |
| batches for the town | 1, near the player | 1, near the player | 1, every plot |

A prefab building averages 212 triangles in the pack (the balconies are 48 a storey on three looks) against a kit building's 2,660 and a kit shell's 2,509, measured over the same 20 block town (`node tools/bench-city.ts --blocks 20 --storeys 4 --dressing kit`). What the kit builds is what a plot's charter asks for, and the word the architecture raises a building under wears a blank frontage, so a plan is the cheapest town the kit ever stands up and a written city of shopfronts and curtain wall costs it several times this. The pane is 19 more a building, derived, and the shell is the same 216 again in the far batch, where a building costs one instance. The kit stays loaded for the ground, the street surfaces, the lamps, the signage and any plot the catalogue has no shape for, but its wall materials are never drawn.

### What a window costs

Measured with `node tools/bench-windows.ts`, which compiles the wall material to the WGSL `WebGPURenderer` would run it as, counts the two arms of the kind decision where they are emitted, and then asks `boxedAt` for every window bay of a planned town's committed geometry.

| | statements | texture fetches |
|---|---|---|
| the whole fragment shader | 326 | 5 |
| a flat window | 11 | 1 |
| a boxed window | 67 | 1 |

A boxed window is 6.1x the flat one's arithmetic and the same single fetch, which is what putting the faces on their own layers buys: the face picks an index, not a region of one picture to compute. On a 6 by 6 block town (309 plots, 228 of them on the pack, the rest raised past the band) the split lands at 6,452 window bays, 37.5% of them shop windows on the pavement: **42.2% march a box and 57.8% are flat panels**, and by glass area 51.0% is marched. The shell path is untouched, because a far window was already a flat lit rectangle.
...
Measured headless in Node on planned towns at density 1 and four storeys, dressed the way the game dresses one, this box over `@gb/kitbash`, with the shell path and with the shell hidden so every plot within reach is dressed whole (`node tools/bench-city.ts --blocks 50 --storeys 4 --mode lod|whole`).

`@gb/scene` opens a city round its own spawn, which on a town nobody has written is its first plot and so a corner of the map. So the ring is measured where a city is dense: the bench stands on the doorstep nearest the middle of town and settles, which takes the whole backlog on one call the way an open does. **Ring** below is that neighbourhood, and it is what a player's own surroundings cost, whatever the size of the town behind them.

| blocks | plots | | open | ring | shells in the ring | buildings in the ring | draws | resident |
|---|---|---|---|---|---|---|---|---|
| 2 | 48 | shell | 189 ms | 35 ms | 48, 9,988 triangles | 48, 12,906 | 5 | 257 MB |
| 2 | 48 | whole | 201 ms | 0 ms | none | 48, 12,906 | 4 | 255 MB |
| 20 | 3,489 | shell | 1,214 ms | 212 ms | 617, 131,024 | 47, 12,380 | 5 | 398 MB |
| 20 | 3,489 | whole | 1,306 ms | 525 ms | none | 617, 163,762 | 4 | 407 MB |
| 50 | 20,233 | shell | 2,400 ms | 109 ms | 620, 130,632 | 42, 11,574 | 5 | 650 MB |
| 50 | 20,233 | whole | 2,400 ms | 254 ms | none | 620, 164,148 | 4 | 655 MB |

A two block town is smaller than the ring, so its whole self is the neighbourhood and it stands up while the city opens. The 50 block figures are a second apart run to run; the rest hold to a few percent.

A 50 by 50 block city is 20,233 plots, every one of them on the pack. It opens in 2.4 seconds and stands as 385k triangles in five draws: the skyline of the whole town, one batch holding the shells within 256 m, and the walls, the glass and the signage of the 42 buildings inside `DETAIL_RADIUS`. Almost none of the open is this box: the rest is `@gb/scene` laying the ground, marking the streets and strewing them.

What the shell path buys is the room raymarch and the pane gone from every building but the near ones, one draw's worth of material, and the emitters of everything past the detail radius never made at all. The same 20 block ring dressed by the kit alone, which is what the plots the pack has no shape for fall back to, is 1.55M triangles of shell and 125k of near building against this box's 131k and 12k, in ten draws against five, and it holds 545 MB against 423 (`--dressing kit`): the pack is what makes a city this size drawable.

**A skyline is boxes, not the kit's bill.** The same 20 block town at the brief's own `maxStoreys` (`node tools/bench-city.ts --blocks 20 --storeys 24`) raises 714 of its 3,489 plots past the band, and the pack has no model for any of them. Their shells are massings drawn here, 26 triangles each, so the middle of a downtown is 617 shells of 33,116 triangles, a quarter of what the same ring costs at four storeys. Their whole buildings are the dressing behind's, and that is the whole bill: the 47 near buildings are 190,316 triangles on the kit's four materials, 265,300 standing in nine draws, 423 MB.

**What walking costs.** Over a 120 m walk across the middle of that 24 storey town, 480 frames of `city.follow` told their own elapsed time, 60 of them a new cell: the median frame is 1 us, the 99th 11.45 ms, and the worst 29.94 ms is one call to the dressing behind for a tower the pack has no shape for. Two frames of the 480 are over 16.7 ms. At four storeys, where every plot is on the pack, no frame of the same walk is: median 2 to 6 us, 99th 3.9 ms, worst 5.6 ms. Walking up to 24 doorsteps across the town, a new neighbourhood streams in over 47 to 89 ms of frames at four storeys and 58 ms at 24, the worst door 607 ms.
...
How a window is laid out and how deep the room behind it runs are the two `WindowKind`s in `src/windows.ts`, and `tools/finishes.ts` reads the same two into the grid the producer is told, so a change to a grid moves the picture with it and a rebuild is needed; a change to `deep` does not. How the bay is cut is `src/bays.ts`, read by the room, the glass and the shell alike. How bright a window burns after dark and how bright at noon is `ROOM` in `src/interior.ts`, and what a player on the pavement then sees of the room against the pane over it is `node tools/measure-glass.ts`; how many windows keep the box is `BOXED` in `src/pick.ts`, how the box is marched and how dark its four shared faces are is `src/roombox.ts`, and how a flat one is read is `src/panel.ts`; what colours a window is lit in is `ROOM_TINTS` in `src/rooms.ts`, which the shell reads too. Every per window choice hangs off one hash in `src/pick.ts`, in the shader and in Node alike, so moving a salt there redraws every window in every city and `tests/glazing.test.ts` pins it. How far the glass stands off the wall, how sharp its reflection is and what it reflects face on are `PANE` in `src/glass.ts`, and what it catches of the street after dark is `STREET` beside it; none of that needs a rebuild, because the panes are derived at load in `src/panes.ts`. What a far building keeps is `src/shell.ts`, what a plot the pack has no model for stands as from far off is `src/massing.ts` alone (the parapet, the bands and the metres a wall repeat is laid over), and what a town of them costs to open, to walk through and to arrive in is `node tools/bench-city.ts`, which loads the pack the way every headless tool and test here does, through `tools/headless.ts` and prints the ten dearest frames of a walk with what each was doing. A balcony is `balcony` in a look, built by `tools/balconies.ts` and held to `BALCONY` in `src/balcony.ts`, and the pack test measures every one; changing any of it is a rebuild. What a building throws onto the street is `src/lights.ts` alone: the lobby's colour and candela, a screen's candela, and how far off a face an emitter stands. What the street face is read as, and how the fascia band is told apart from a balcony slab, is `src/face.ts`; where a fixture the kit wrote is seated on it is `src/fixtures.ts` alone, and `node tools/measure-fixtures.ts` prints what the pack's own street lands at. Neither needs a rebuild: both read the geometry the plot is drawn with. Adding or replacing a room, a flat panel, a shared face or an ad is the theme pack and nothing else: the prompt is in `docs/textures/PROMPTS.md`, the image goes in the pack's own folder, the name and its `where` go in `theme.json`, `node tools/draw-rooms.ts <folder of raw images>` crops and sizes whatever raw room images are in a folder, and `node tools/theme-buildings.ts` restacks the two strips and rewrites the manifest with no producer run. Nothing in `src/` names a picture, so nothing in `src/` changes.
