 ...
 * streets, the stations, the time and the file are all the plan's.
 *
...
console.log('| blocks | grid | metres | avenues | buildings | stations | plan | world file |')
console.log('|---|---|---|---|---|---|---|---|')
...
for (const blocks of sizes.split(',').map(Number)) {
  ...
  console.log(
    `| ${blocks}x${blocks} | ${world.grid.width}x${world.grid.height} | ${Math.round(world.grid.width * world.cellSize).toLocaleString('en-GB')} | ${avenues} | ` +
      `${world.plots().length.toLocaleString('en-GB')} | ${world.stations().length} | ${(took / 1000).toFixed(2)} s | ${(file / 1e6).toFixed(2)} MB |`,
  )
}
