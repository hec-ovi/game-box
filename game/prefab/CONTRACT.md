...
- `@gb/forge` contract, in the tests and the tools and nowhere in `src/`: `Forge.plan`, the arithmetic half of a city, is the town every measurement here is taken over. A shell, a window and a seated sign are all read off a plot's footprint, its height and its charter, none of which anybody writes, so nothing here needs a city that has been written.
...
plots, every one on the pack, 340 door lamps and 322 flat plates. Both columns
come off one run: every fixture is read against the face the model drew, once
where the kit wrote it against the plot and once where it was seated.
...
| plates in the same plane as that face | 61 of 322 | 0 |

The plate written on a fascia band is the one thing still not right: 56 of them
land on a band and 16 are a 0.758 m plate on a 0.600 m band, because the kit
...
Measured headless in Node with `node tools/measure-city.ts`, a planned 4 by 4 block town (170 plots, every one on the pack), the way `@gb/scene` batches it:

| | walls | glass | shell |
|---|---|---|---|
| triangles a building | 197 | 13 | 197 |
| batches for the town | 1, near the player | 1, near the player | 1, every plot |

A prefab building averages 212 triangles in the pack (the balconies are 48 a storey on three looks) against a kit building's 13,494 and a kit shell's 10,846, measured over the same 20 block town (`node tools/bench-city.ts --blocks 20 --storeys 4 --dressing kit`). The pane is 13 more a building, derived, and the shell is the same 197 again in the far batch, where a building costs one instance. The kit stays loaded for the ground, the street surfaces, the lamps, the signage and any plot the catalogue has no shape for, but its wall materials are never drawn.
...
|---|---|---|---|---|---|---|---|---|
| 2 | 48 | shell | 189 ms | 35 ms | 48, 9,628 triangles | 48, 12,464 | 5 | 257 MB |
| 2 | 48 | whole | 201 ms | 0 ms | none | 48, 12,464 | 4 | 255 MB |
| 20 | 3,489 | shell | 1,214 ms | 212 ms | 617, 122,516 | 47, 12,582 | 5 | 398 MB |
| 20 | 3,489 | whole | 1,306 ms | 525 ms | none | 617, 159,450 | 4 | 407 MB |
| 50 | 20,233 | shell | 2,400 ms | 109 ms | 620, 123,196 | 42, 10,700 | 5 | 650 MB |
| 50 | 20,233 | whole | 2,400 ms | 254 ms | none | 620, 161,598 | 4 | 655 MB |
