...
| `STREAM_BUILDS` | 1 | how many buildings a ring dresses on a frame that was told its own elapsed time |
...
- **A ring takes its queue over the frames, not on one.** Building one shipped-kit tower and copying it into a batch is tens of milliseconds, and a step across a cell brings several buildings into a wide ring, so `follow` told the frame's own elapsed time builds `STREAM_BUILDS` of them and leaves the rest for the next frame; a plot waiting its turn stands as its massing meanwhile. `follow` told no `seconds` takes the lot, which is what a city opening at the spawn wants and what a ride between stations wants behind its veil. One a frame is well ahead of the demand: on the town the game builds, a 256 m ring takes about a thirtieth of a building a frame at walking pace and about half at 20 m/s.
