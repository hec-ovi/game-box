...
Standing at a station (`boarding` names it), that station wears `Here` and every other carries a Travel button that reports `travel` with its id. The list always says which of the situations the player is in, so a heading never stands over rows that answer nothing: a town with no stations says so, a player away from them is told to walk up to an entrance, and a player standing at the only station there is reads that there is nowhere to ride. The ride is the game's: it closes the map, pushes a `loading` with the title alone as the veil, moves the player and takes the veil away.
