...
- The detail batches are their own buffers, cut at open with room past the neighbourhood of the spawn, so the churn of buildings coming and going never touches the town-sized shell buffers and a walk through the town rarely has to grow them. The shell buffers are cut to fit, because every plot goes into them at open and `add(plot)` is the only thing that follows. A building taken out leaves its range behind; it is packed away only when the next one would not fit, so a batch settles at the size of what stands in it at once. The price is one more draw per material near the player: the greybox pays seven at the spawn of a twenty block city, a kit pays as many materials as it draws a building with.
