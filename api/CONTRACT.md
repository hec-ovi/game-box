...
|---|---|---|
| `POST /v1/chat/completions` body | [schema/chat-request.json](schema/chat-request.json) | JSON body; `messages` non-empty; `stream` optional |
...
|---|---|---|
| chat response (non-streaming) | [schema/chat-response.json](schema/chat-response.json) | single assistant message with finish_reason |
| chat SSE chunk (streaming, each `data:` payload) | [schema/chat-stream-event.json](schema/chat-stream-event.json) | token chunks with `finish_reason: null`, one closing chunk with a finish_reason, then literal `data: [DONE]` |
