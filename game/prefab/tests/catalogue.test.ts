...
describe('the catalogue', () => {
  ...
  it('gives a plot a model of its own shape', () => {
    const design = catalogueOf().design(plotOf(), { width: 8, depth: 12 }, ['shop'])
    expect(design?.model).toMatch(/8x12x2$/)
  })

  it('reads the shape in the door’s frame, so a turned plot is the same shape', () => {
    const catalogue = catalogueOf()
    const facing = catalogue.design(plotOf({ entrance: { cell: { x: 6, y: 3 }, facing: 'north' } }), { width: 8, depth: 12 }, ['shop'])
    const turned = catalogue.design(plotOf({ rect: { x: 4, y: 4, w: 6, h: 4 }, entrance: { cell: { x: 3, y: 6 }, facing: 'west' } }), { width: 12, depth: 8 }, ['shop'])
    expect(facing?.model).toBe(turned?.model)
  })

  it('gives the same plot the same building however often it is asked', () => {
    const catalogue = catalogueOf()
    const answers = new Set(Array.from({ length: 20 }, () => JSON.stringify(catalogue.design(plotOf(), { width: 8, depth: 12 }, ['shop']))))
  ...
  it('does not give every plot the same building', () => {
    const catalogue = catalogueOf()
    const seen = new Set(
      Array.from({ length: 40 }, (_, i) => catalogue.design(plotOf({ id: `plot_${i}`, kind: 'chapel' }), { width: 8, depth: 12 }, ['chapel'])?.model),
    )
    expect(seen.size).toBeGreaterThan(1)
  })

  it('gives a charter a model that shares a tag with it, and the whole shape when none does', () => {
    const catalogue = catalogueOf()
    const size = { width: 8, depth: 12 }
    for (let i = 0; i < 20; i++) {
      expect(catalogue.design(plotOf({ id: `plot_${i}`, kind: 'house' }), size, ['background', 'house', 'masonry', 'narrow'])?.model).toBe('home-8x12x2')
    }
    // a word no look has heard of, whose charter says it is a kind of shop
    for (let i = 0; i < 20; i++) {
      expect(catalogue.design(plotOf({ id: `plot_${i}`, kind: 'bodega' }), size, ['bodega', 'cafe', 'narrow'])?.model).toBe('shop-8x12x2')
    }
    // nothing claims a chapel, so the whole shape answers rather than nothing
    expect(catalogue.design(plotOf({ kind: 'chapel' }), size, ['chapel', 'landmark'])).toBeDefined()
    // and a tag two looks carry draws from both
    const shops = new Set(Array.from({ length: 30 }, (_, i) => catalogue.design(plotOf({ id: `s_${i}`, kind: 'shop' }), size, ['shop'])?.model))
    expect([...shops].sort()).toEqual(['shop-8x12x2', 'works-8x12x2'])
  })

  it('refuses a tag that is not a lowercase word, and no word it has not heard of', () => {
    const model = catalogueOf().models[0]!
    expect(() => catalogueOf({ models: [{ ...model, tags: ['Bar'] }] })).toThrow(InvalidCatalogue)
    expect(catalogueOf({ models: [{ ...model, tags: ['speakeasy'] }] }).models[0]?.tags).toEqual(['speakeasy'])
  })

  it('starts two plots on one model at different rooms', () => {
    const catalogue = catalogueOf()
    const rooms = Array.from({ length: 40 }, (_, i) => catalogue.design(plotOf({ id: `plot_${i}`, kind: 'house' }), { width: 8, depth: 12 }, ['house']))
  ...
  it('says nothing for a shape it does not hold, so the kit can answer', () => {
    expect(catalogueOf().design(plotOf({ storeys: 9 }), { width: 8, depth: 12 }, ['shop'])).toBeUndefined()
    expect(catalogueOf().design(plotOf(), { width: 30, depth: 12 }, ['shop'])).toBeUndefined()
  })
  ...
  it('names the charters no look claims', () => {
    const catalogue = catalogueOf()
    expect(catalogue.suits([{ word: 'house', suits: ['background', 'house'] }])).toEqual({ ok: true })
