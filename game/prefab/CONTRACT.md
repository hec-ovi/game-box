...
Measured headless in Node on forged towns at density 1 and four storeys, dressed the way the game dresses one, this box over `@gb/kitbash`, with the shell path and with the shell hidden so every plot within reach is dressed whole (`node tools/bench-city.ts --blocks 50 --storeys 4 --mode lod|whole`). `@gb/scene` streams both rings, so what is built at open is the plots within `SHELL_RADIUS` and `DETAIL_RADIUS` of the spawn and not the town:

| blocks | plots | | open | in the dressing | shells built | near buildings | draws | resident |
|---|---|---|---|---|---|---|---|---|
| 2 | 48 | shell | 171 ms | 37 ms | 48, 9,708 triangles | 22, 5,764 | 5 | 249 MB |
| 2 | 48 | whole | 145 ms | 35 ms | none | 48, 12,934 | 4 | 266 MB |
| 20 | 3,489 | shell | 1,028 ms | 89 ms | 415, 84,488 | 31, 8,270 | 5 | 390 MB |
| 20 | 3,489 | whole | 1,094 ms | 150 ms | none | 415, 111,336 | 4 | 403 MB |
| 50 | 20,233 | shell | 3,661 ms | 37 ms | 170, 34,488 | 20, 5,266 | 5 | 767 MB |
| 50 | 20,233 | whole | 4,472 ms | 103 ms | none | 170, 45,992 | 4 | 776 MB |

A 50 by 50 block city is 20,233 plots, every one of them on the pack. It opens in 3.7 seconds and stands as 283k triangles in five draws: the skyline of the whole town, one batch holding the shells within 256 m, and the walls, the glass and the signage of the 20 buildings inside `DETAIL_RADIUS`. Almost none of the open is this box: the rest is `@gb/scene` laying the ground, marking the streets and strewing them.

What the shell path buys is the room raymarch and the pane gone from every building but the near ones, one draw's worth of material, and the emitters of everything past the detail radius never made at all. The same town dressed by the kit alone, which is what the plots the pack has no shape for fall back to, is 215.9M triangles and 27 GB (`--dressing kit`): the pack is what makes a city this size drawable.

**A skyline is boxes, not the kit's bill.** The same 20 block town at the brief's own `maxStoreys` (`node tools/bench-city.ts --blocks 20 --storeys 24`) raises 714 of its 3,489 plots past the band, and the pack has no model for any of them. Their shells are massings drawn here (26 triangles each), so the far field costs what it costs at four storeys: 415 shells of 78,230 triangles, 128,368 triangles standing, five draws, 403 to 419 MB. Their whole buildings are still the dressing behind's, near the player only. What the kit's stack costs instead, measured over 80 of those plots: 7,439 triangles and 2.98 ms for a shell, 26,027 triangles for the building.

**What walking costs.** Over a 120 m walk from the spawn of that 24 storey town, 480 frames of `city.follow` told their own elapsed time, 60 of them a new cell: the median frame is 4 to 6 us, the 99th 10 to 15 ms, and the worst is one call to the dressing behind for a plot the pack has no shape for. Standing at a door in the downtown, a new neighbourhood streams in over 69 to 89 ms of frames (524 to 653 at the worst door).
