...
export interface GroundTexture {
  ...
  /**
   * Millimetres of real surface one texel of its maps covers, at the size the
   * pack stores them. What a derived map was aimed at, and what says whether a
   * feature can be in it at all: nothing under a texel survives a mip.
   */
  readonly texel: number
  /** Roughness its wear image runs between, which is what the surface is made of. */
...
export interface GroundLook {
  ...
  /**
   * The tiling occlusion and roughness image, when it has one. Where a surface
   * takes one, its roughness is the material's own, texel by texel: a wheel
