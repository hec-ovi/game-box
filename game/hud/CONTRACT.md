...
| `patch.inspecting` | [Inspecting](src/types.ts) | the thing open in the inventory: its `itemId` and `frames`, the views of it from all the way round in order, starting face on. An empty `frames` says which thing it is while the game is still drawing it; `null` closes it |
...
| Map | `map`, `objectives`, `trackedQuestId`, `quests` | `travel`, `district` |
...
A district is drawn as the shape it is, not as a box: its blocks are filled as one region and the border is every cell edge with nothing of the same district behind it, so an L, a Z or a T reads as one shape with one outline and no line through the middle of it. Its name is written across its widest block. Under the pointer it lights and the rest step back, and clicking one reports `district` with its id, which is the player asking to be pointed at it. Past zoom 3 the plan is showing streets and the districts fade out, because a border across a street is in the way of reading it. A city that was cut into none draws none.
