...
4. **A build cannot be planned without being run.** `planStreets`, `planRaise`,
   ...
   (Row 306.)
5. **A world file cannot give back the input it was written from.** It does not
...
6. **A refresh with the model on wipes the playthrough.** Reproducibility cannot
   ...
   `bundle`.
7. **Tool calls through OpenRouter are unproven.** A forced call came back with
...
8. **The plot shape band is written down twice.** `FRONTS` / `DEPTHS` live in
...
9. **Two fifths of the street face is the producer's plain finish.** On the
...
10. **Minigames, and a score that survives.** Two games on a machine screen
    exist; nothing else does.

## His own list, still open

11. **Voice.** He has it solved in another project and wants it here.
