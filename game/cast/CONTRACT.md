...
The pack models one pair of eyebrows per head, so the second shape a person can be given is the other head's. A pair is a brow row and a lash row in one sheet, and where it sits on the face is what tells them apart, so it is put over the body's own pair before it is settled (`tools/wardrobe/face.mjs`). Left where the head bone carries it, the borrowed pair lands 7 mm off this body's brow ridge and 10 mm out of the plane of its face, and the settle pushes it up the forehead until the lash row clears the eye and reads as a second pair of brows.
...
- **A person drops a shadow and takes one.** Every mesh on the body and on everything worn or held says it casts and receives, because the renderer only draws what says so. A room is lit by its own fixtures with two of them casting, so the furniture shadows the floor; without this the person standing behind the counter did not, which reads as somebody pasted onto the picture rather than standing in it. A skinned mesh is skinned again per shadow face, so what it costs on a street full of people is worth measuring before the number of casters outdoors goes up.
- **Every pair of eyebrows is on the brow ridge.** A person carries two shapes and is shown one; whichever it is, it sits within 3 mm of the height of the pair the artist put on this head, measured off the eyes in `tests/looks.test.ts`. A pair left where the head bone carries it rides up the forehead when it is settled out of the skin, and its lash row clears the eye as a second pair of brows.
...
- Everybody has hair, or is bald on purpose: the hairstyle, the eyebrows, the beard and their colours are drawn from the NPC's id. The hair and the beard may be out of a bottle, and this city dyes its hair; the eyebrows never are. A pair of brows is one sheet holding a brow row and a lash row, so whatever colours the brows colours the lashes with them, and dyed to match the hair somebody with acid green hair wore acid green eyelashes. They take a grown colour instead, their own where the hair grew that way and one off their id where it did not. Every piece sits outside the skin, so a buzz cut shows as a buzz cut. Tinted materials are shared across the whole cast, so a crowd costs one material per colour per hair texture, not one per person.
