...
| Map | `map`, `reading`, `quests` | `read`, `map-move`, `travel` |
...
- Every list on the map has words for having nothing in it, and they are the corner panel's words, so a player who has taken no job reads the same sentence in both places.
