...
import { Route, walkLength } from './route.ts'
...
export class Guide {
  ...
  resolve(): CompassGoal | undefined {
    return this.way()?.goalFrom(this.#from())
  }

  /**
   * The whole walk, corner by corner, so whoever is drawing it can read the
   * bearing and the metres left off it every frame without asking `@gb/nav`
   * again. Finding a way through the streets is the expensive half and it
   * holds while the player walks it; where they are along it is arithmetic and
   * goes stale in a stride.
   */
  way(): Route | undefined {
    const goal = this.#goals()[0]
    if (!goal) return undefined
    const corners = this.#route(this.#from(), goal)
    if (!corners) return undefined
    return new Route({ label: goal.label, corners, ...(goal.line ? { line: goal.line } : {}) })
  }
  ...
  metresTo(to: { readonly x: number; readonly z: number; readonly plotId?: string }): number | undefined {
    ...
    return path ? walkLength(this.#nav.waypoints(path)) : undefined
  }
