...
Turns a city into something you can stand in: ground, marked streets, buildings and interiors as three.js objects, at the size and place the world says, lit by what the buildings throw onto the street and, indoors, by the fixtures the room's own art drew. The whole town is batched as its skyline; the buildings within a few blocks of the player wear the shell their dressing drew and the ones on the street wear its whole detail, rooms are built on entry and let go when far, and where a visitor may stand in a room is published.
...
| `CityOptions` | `seed?`, `wetness?`, `night?`, `clutter?`, `lights?`, `detail?`, `shell?` | left out: the world's own seed, a dry street after dark, the default amount of rubbish, `LIVE_LIGHTS` live lights, `DETAIL_RADIUS` metres of detail and `SHELL_RADIUS` metres of shell. `clutter: false` sweeps the streets. `shell` never reaches less far than `detail`: asked for less, it takes the detail's own reach |
...
| `city.follow(x, z, seconds?)` | metres on the ground: where the player is, and the frame's own elapsed time | every frame. The emitters nearest it become the live lights, fading in over `FADE_SECONDS` (0.7) and out again when they lose one, or landing at once when no `seconds` is given; when its cell changes, the buildings that went beyond `shell` or `detail` metres drop to the coarser look at once and the ones that came within are queued; the queue is worked through for as long as the frame's budget lasts and never further; rooms beyond `detail` are let go |
| `city.settle()` | nothing | everything `follow` has queued and not built, built now, however long it takes, and the lights cut again over what it built: what a loader and a veil are for. A frame in a running game never calls it |
...
| `InteriorBuild.lights.lit(fixtures)` | `LightEmitter[]` in the interior's own metres | what the room's art drew: the lit channel over its walls, its strips, niches, panes, screens and dance floor. Handed nothing, the plain lamps the room was built with stay |
| `InteriorBuild.lights.follow(x, z, seconds?)` | where the player stands in the room's own metres, and the frame's own elapsed time | every frame the player is inside. The fixtures nearest them become the live lights, fading over `FADE_SECONDS`; no `seconds` lands them at once |
| `Dressing` | `building`, `shell?`, `lights?`, `prop`, `character`, `pickup`, `ground`, `surface`, `marking?`, `clutter?`, `massing?` | every object it returns has its origin at the centre of its base, faces north (-Z) unturned, and is its own: an object handed over a second time is drawn once, where it was placed last |
| `Dressing.building(plot, size, charter)`, `Dressing.shell?(plot, size, charter)`, `Dressing.lights?(plot, size, charter)` | a `@gb/world` `Plot`, `BuildingSize` (`width`, `depth`, `height` in metres), the plot's `ResolvedCharter` | the charter is `world.charter(plot.kind)`, and a plot's kind is the word of a charter the world holds, so it is always there. `shell` is the same building as seen from down the street: walls and roof, nothing that is only worth drawing near (no signs, screens or rooms behind the panes); left out, `building` is what the far ring draws. `lights` is asked after `building` for that plot, never after `shell`. None of them has to answer: a member that is there and answers nothing, or an object with nothing in it to draw, reads as the member being left out for that plot |
...
| `CityBuilding` | `plotId`, `bounds`, `visible`, `step`, `detailed` | the box it occupies in city metres (its footprint by its height, whatever its dressing drew inside that), a switch that takes it out of the city or puts it back with no rebuild, and which of the three ways it is drawn right now |
| `BuildingStep` | `massing`, `shell`, `detail` | coarsest first: the box it occupies in its charter's colour, the shell its dressing drew, the whole building |
| `add(plot)` | `CityBuilding` | one more plot built into the city that is already standing, at whatever step its distance from the player says. Its ground is not repainted: the grid changed after the ground was laid, and the building covers it |
...
| `SHELL_RADIUS` | 256 m | how far from the player a building wears the shell its dressing drew, four times the detail and about four blocks of a forged city; past it the skyline carries it |
| `STREAM_BUDGET` | 4 ms | how long a frame that was told its own elapsed time may spend dressing and batching buildings while the player walks |
| `STANDING_BUDGET` | 12 ms | the same while the player has not moved since the last frame |
| `STREAM_DEBT` | 48 ms | the most overrun the streaming carries into the frames after it |
...
| skyline mesh | `root.children` named `city:massing` | one `THREE.BatchedMesh` for the whole town: one unit box of geometry per charter colour, one instance per plot scaled to its footprint and height, hidden while that plot wears a finer step. It casts no shadow and takes none |
| shell meshes | `root.children` named `city:<material>` | one `THREE.BatchedMesh` per material the shells are drawn with; the buildings within `shell` metres come and go from these as the player moves |
...
| `Dressing.massing?()` | one `THREE.Material` | one material for the whole skyline. The charter's `tint` rides on the vertices, so it is asked for once and never per building; left out, the skyline gets a plain matte one |
...
| `buildInterior` | `{ root, anchors, props, people, pickups, lights, leave, blockers, visitorCells, entrance, inward, dispose }` | floor, walls with the doorways cut out, ceiling, the room's own light under `root`, furniture standing where the world puts it (on the floor, or on the top it was lifted onto), an empty object at every anchor carrying its kind |
| `InteriorBuild.lights` | `RoomLight`: `group`, `fixtures`, `lights`, `lit`, `follow`, `dispose` | every fixture the room carries, and the `ROOM_LIGHTS` point lights the budget allows. Built on one plain lamp over each of the interior's rooms, so a room no art dressed is dim and never black |
...
| `ROOM_LIGHTS`, `ROOM_SHADOWS` | 20, and `{ casters: 2, mapSize: 512, normalBias: 0.024, softness: 2 }` | how many of a room's fixtures are real lights at once, and how many of those cast; both measured below |
| `Shadowing` | `casters`, `mapSize`, `normalBias`, `softness` | what `CityLights` is told about shadows; the first `casters` slots cast and it never moves, because `castShadow` is part of the node graph every lit material compiles against |
| `BOUNCE`, `CEILING_HEIGHT` | 0.12, and `METRICS.building.groundFloorHeight` (4 m) | the share of a room's own light that comes back off its surfaces, as one hemisphere named `bounce`; how tall a room is |
...
- The buildings are drawn out of one `THREE.BatchedMesh` per material, not one object each, so the city costs a draw per material however many buildings it has, plus one for the skyline. Every building keeps its own transform, its own bounds and its own visibility inside the batch, so three still culls them one at a time and submits only what the frustum reaches, in the shadow pass as well as the frame. That is the whole reason it is a batch and not a merge: a merge costs the same one draw and hands the entire town to the rasteriser every time.
- **A batch that grows says so.** Growing a `BatchedMesh` replaces the textures an instance's matrix and its place in the draw order are read from, and replaces the geometry under it. A renderer reads those textures through the material's own program and looks at them again only when that material's version moves, so a batch that grew while it was on screen would go on drawing through the copies it threw away: a building placed after the growth is drawn from a matrix that is not there, and the buildings already in it from whichever matrix the stale draw order points at. That is a building blinking out with its neon and its lamps still burning over the gap. So growing moves the material's version, and packing (which keeps the buffers it has) does not. Measured on a three block town dressed the way a kit dresses one, walked out and back: 14 growths on screen, none of them announced; 1, announced, once the buffers carry room.
...
- **A room is lit by its own fixtures.** Whoever dresses a room draws lit things into it: a channel under the wall rail, light lines up the bays, a strip under each niche head, panes, the glass of each machine, a lit floor. Emissive geometry draws itself and lights nothing, so the art also publishes a `LightEmitter` standing in each of them, in the interior's own metres, and `RoomLight.lit(fixtures)` hands them to the same budget the street's lamps run on: every fixture is kept, `ROOM_LIGHTS` (20) of them are `THREE.PointLight`s at `decay` 2 with the fixture's own reach as their cutoff, and the nearest to the player get those. Held at full night, because a ceiling does not care what the sky is doing.

  This replaces one flat fill that was the same light from the same direction in every room in the city. What it buys, measured over a town's twelve interiors, standing in each of its thirty-six rooms in turn: a floor's irradiance runs from 0.36 at the fifth percentile to 4.11 at the ninety-fifth, an eleven to one spread where the fill gave every point of every room the same number. The medians are 1.9 on a floor and 1.5 on a wall at eye height, against 3.0 and 0.97 to 1.26 flat, so the exposure the app develops a room at did not have to move.

- **Twenty lights, two of them casting.** Both numbers are the point where the budget stops being visible or starts being expensive. Twenty: the room's own floor drifts 1.13 times between the brightest and the dimmest place to stand, against 1.31 at sixteen and 1.07 at twenty-four, and a fifth of a stop over a walk that fades in 0.7 s is not something a player sees. Two casters: a shadow casting point light is six renders of the room a frame, one per cube face, each culled to its own 90 degrees, and drawing only what casts, which measures at 2.1 times the room's meshes in extra draws per caster (a room is 22 to 42 meshes), so the pair is 90 to 180 depth-only draws over a room that costs 34 to draw. 512 is memory rather than sharpness: a caster's cube is 12.6 MB in colour and depth and 1024 would be 100 MB for the pair, while 512 puts a shadow texel at about 12 mm at the 3 m a ceiling fixture stands off the floor under it, which is four texels across a chair leg and a soft contact shadow with the five sample filter over them.

- **A room bounces off itself.** The one thing left that is not a fixture is `bounce`, a hemisphere carrying `BOUNCE` (0.12) of what the room's own fixtures lay on its floor, in the colour of those fixtures. It is what keeps a face turned away from every one of them off pure black and it does nothing else: a closed room returns `p / (1 - p)` of its direct light, and the interior palettes reflect 0.074 to 0.27, mostly near 0.09, which puts the indirect share at about a tenth. It is retuned from the fixtures, so a room with one lamp in it bounces like a room with one lamp in it.

- **No room is ever black.** A room built before its art has said anything stands on one plain lamp over each of its rooms, hung under the ceiling, the same way a greybox building carries one lamp over its door. Its candela is charged by the square metre of the room it hangs in, because one lamp over a big room is not one lamp over a small one, and the rate lands a plain room where a dressed one lands: a floor median of 1.7 to 2.0 measured over two towns' interiors, against the 1.9 a dressed one gets. `lit(fixtures)` swaps them for the art's own; told nothing, they stay.
...
- **Every sign, lamp, screen and lit lobby is an emitter, and a few of them are lights.** A dressing publishes `LightEmitter`s from `lights(plot, size, charter)`; this box carries them into city metres and keeps every one for as long as the building stands in detail. Every lit material pays for every point light in the frame, so `LIVE_LIGHTS` (16) `THREE.PointLight`s are made once and the nearest emitters to the camera get them; the rest stay emissive geometry until the camera comes closer. `follow(x, z, seconds)` is the whole of it, and a city is built with the lights round the spawn so the first frame is lit before the app has said where the camera is.
- **A light arrives and leaves over time, not on a frame.** Handed an emitter it comes up over `FADE_SECONDS`, and it goes down over the same before it will take another, so a walk brings the lamps up ahead instead of switching them on in front of the player. It keeps the emitter it holds until that one is `HOLD` past the edge of the budget, so two lamps either side of that edge do not trade a light back and forth, and the budget is cut again only once the camera has moved `RESCAN` metres, because cutting it measures every emitter in the city. `follow` told no `seconds` skips all of it and lands at once, which is what opening a city at the spawn wants.
- **A building's lights are never in the city while the building is not drawn as the thing that throws them.** Emitters are hung when the building that throws them actually put something in the city and taken out with it, so a plot the dressing draws nothing for hangs nothing over an empty pavement. A building past the detail radius throws none, and a near one that drew nothing throws none either.
...
### How near decides how a building is drawn

- **A building is drawn at every distance, whatever its dressing publishes.** A dressing decides what a building looks like; it never decides whether there is one. Every plot in the world stands in the skyline from the moment the city opens, so an answer this box cannot draw is read as no answer for that plot and the next thing that can be drawn stands there instead: a `shell` that answers nothing leaves the plot with its whole `building` as its far look, a `building` that answers nothing leaves the near plot standing as the shell it already had, and a dressing that draws nothing at all leaves the plot standing as its massing. Measured on two generated towns, walked cell by cell and turned on the spot at each step, judged by what a camera would put in the draw rather than by what a ray can hit: a dressing that answers `undefined` through `shell`, one that answers an empty object through `shell`, one that answers an empty object through `building` and one that draws a building out of several materials all leave every plot the frustum reaches in the draw at every step.
- **Three steps, and the player's cell picks one.** The massing is the box the plot occupies in its charter's `tint`, twelve triangles, and every plot has one for the life of the city. `Dressing.shell` is the walls and the roof, batched into `city:<material>` for the plots whose footprint comes within `shell` metres of the middle of the player's cell. `Dressing.building` is the whole building, batched into `detail:<material>` for the plots within `detail` metres. A building is drawn at the finest step it is standing at and the coarser ones are hidden, so a building is one draw, out of one batch, at any distance, and `plotOf` names the plot from any of them.
- **The skyline is what makes the shells streamable.** The far half of a town is standing whatever the player is doing, so a shell that has not been built yet, or has been let go, is a building drawn more coarsely and never a hole in the street. That is why the massing is held for the whole town rather than streamed with the rest: it costs a unit box per charter colour and an instance matrix per plot, which is nothing to hold and nothing to build, and it is the thing the streaming can fall back on.
- **What is drawn is a pure function of the player's cell.** `follow(x, z)` does nothing to the buildings until the cell changes; then every plot is measured against the new cell, what went beyond a ring is taken out at once with the emitters it carried, and what came within is queued in the order the world lists its plots. So two players standing in the same cell of the same city see the same buildings, and walking away and back rebuilds the same town.
- **A ring spends time, not turns.** One building is not one cost: on the metro 20 by 20 city, a shell out of the shipped pack is 0.1 to 0.7 ms and a plot the pack has no shape for, dressed whole by the kit, is 8 to 55 ms. So `follow` gives the rings `STREAM_BUDGET` (4 ms) to spend, or `STANDING_BUDGET` (12 ms) where the player has not moved since the last frame, and each ring stops as soon as that is gone. A build already started is finished, because a half-built plot is a hole in the street, and what it went over by is carried: the frames after it pay the overrun down out of their own budget and build nothing until it is clear, up to `STREAM_DEBT` (48 ms), past which the rest is written off. The near ring goes first, so a building the player can read beats one four blocks off. A plot waiting its turn stands as its massing or its shell meanwhile. `settle` takes the lot, which is what a loader wants and what a ride between stations wants behind its veil; `follow` never does, whether or not it was told an elapsed time, because a frame in a running game has one budget and no way of knowing what is queued behind it.

  What it buys, measured on the metro 20 by 20 city at 24 storeys over a 120 m walk (480 frames, 60 of them a new cell): the 99th percentile frame goes from 15 to 21 ms down to 10 to 15, the median from 36 to 66 us down to 4 to 6, and the worst frame is one dressing call rather than a call plus everything queued behind it. Standing at a door in the downtown, the streaming a new neighbourhood costs falls from 174 to 244 ms of frames (843 to 1,069 ms at the worst door) to 69 to 89 (524 to 653), because the shells round it are boxes now rather than kit stacks.
- **The lights come with the detail.** `lights` is asked after `building`, so a shell has no emitters and a building past the detail radius throws none, which is the budget the frame already lived with: the nearest sixteen emitters were never further than a street away. A dressing with no `shell` has no near ring at all, and there the far ring draws the whole building and carries its lights.
- The shell and detail batches are their own buffers, cut with room past the neighbourhood of the spawn, so a walk through the town rarely has to grow them. A building taken out leaves its range behind; it is packed away only when the next one would not fit, so a batch settles at the size of what stands in it at once. Measured on a 20 by 20 block town walked 120 m: 4 packs, none over half a millisecond, and no growth. The price of the split is one draw for the skyline and one more per material near the player, whatever the size of the town.
- A city with `add(plot)` called after open puts the new plot in the skyline and then at whatever step the cell `follow` last saw puts it, spawn included.
...
- The models are generated here, not downloaded: a wheeled bin, a skip with a load in it, a crate, a leaning pallet, a bin bag put out whole, another split and trodden flat, a coil of cable, a scrap of paper and a crushed can, in two to four colourways each. Every one is brought inside the footprint its kind publishes as it is built, so nothing hangs over a rectangle the game tells everyone else is clear.
- **A street is bags before it is anything else.** A `bag` is a sack standing where it was put out; a `sack` is the same one split open and trodden flat, 18 cm high so it lies with the litter and is walked over. Both are painted as black polythene, near black with the faint green or blue cast the plastic has, and the material carries enough sheen for a wet street's lamps and signs to find them. What a pavement is made of, measured on a five block town: bags 55.3% (a flat `sack` 39.3%, a standing `bag` 16.0%), `scrap` 27.3%, `can` 7.2%, `crate` 4.4%, `bin` 2.7%, `cable` 2.0%, `pallet` 0.7%, `skip` 0.4%. Timber and metal keep their own lightness (`pallet` 0.31 to 0.32, `can` 0.38 to 0.56); the polythene sits at 0.04 to 0.07 and a scrap of sodden card at 0.17 to 0.20.
...
About 0.08 ms a light on this screen, whichever backend, and it is per lit pixel rather than per building: a kit that costs more per fragment pays its own base and the same light term over it. Sixteen is 1.3 ms for the doorlamps, signs and screens of about three buildings round the camera, which is the end of a street canyon you are standing in; it is the budget, and `CityOptions.lights` moves it for a machine that wants another. `node game/scene/tools/bench/measure.ts street 0 4 8 16 32` is where the table comes from (`CHROME` names the browser, vite serves the repo root on 5312), `measure.ts room 0 2 4` reads a room's floor back from straight above and times standing in it at that many shadow casters, and `SHOT=<dir>` keeps a screenshot of each run.

### What the far field costs

Measured headless in Node on a 20 by 20 block city dressed in the shipped Downtown kit under `@gb/prefab`'s pack: 3,489 plots on 547 by 563 cells, 106 of them over the plot band, the tallest 24 storeys. The camera stands at the spawn with the far plane the game gives it, which comes off the air rather than off the size of the map and is kilometres, so the whole town is inside it and what a heading does not reach is what the frustum throws away. `shells to all` puts a shell on every plot, which is what a city cost before the skyline carried the far field. The walk is 120 m in 25 cm steps, 480 frames each told its own elapsed time, 60 of which cross a cell:

| shells reach | open | rss | triangles held | drawn facing the door | building draws | walk, 99th frame | worst frame |
|---|---|---|---|---|---|---|---|
| all | 1,776 ms | 1,021 MB | 4,944,603 | 4,439,633 | 10 | 0.82 ms | 1.30 ms |
| 512 m | 1,061 ms | 647 MB | 2,076,428 | 1,670,906 | 11 | 12.75 ms | 30.66 ms |
| 256 m | 492 ms | 425 MB | 185,949 | 130,349 | 11 | 4.19 ms | 5.76 ms |
| 128 m | 471 ms | 386 MB | 71,978 | 43,198 | 6 | 2.81 ms | 15.67 ms |

At the published 256 m the town holds a thirteenth of the geometry and submits a fourteenth of it, opens in a third of the time and in 42% of the memory. The skyline is 41,868 triangles of that, twelve a plot, and it does not move with the reach. Turning on the spot from the spawn at 256 m the four headings submit 36,156, 1,876, 66,678 and 130,349 triangles; with a shell on every plot the same four are 435,679, 1,876, 1,306,545 and 4,439,633. The rubbish is charged to the streets and is the same either way: 649,626 held, 508,186 in the draw.

**The frame is bought back on the walk.** A shell on every plot is built once at open, so a walk costs nothing; streaming pays for it a building at a time, and the worst frame of a walk is whatever the slowest single `Dressing.shell` call costs. It was 48 to 53 ms, because a shipped-kit tower assembled every kit piece of every storey: 88,839 triangles and 36 ms in the dressing with 18 ms copied into the batch. `@gb/kitbash` 0.15.0 draws a tall shell as its shopfront and roof with the plain module's outer face carried up the wall between them, which is 5,943 triangles built in 1.4 ms, and the walk's worst frame went with it. The batch was never the cause: it did not grow on that walk and packed four times, none of them over half a millisecond. `node game/prefab/tools/bench-city.ts --blocks 20 [--shell 256|all]` is where the table comes from.

The same 20 by 20 town greyboxed, which needs no pack, on `node game/scene/tools/bench/headless.ts 20`. `kitted` is a greybox whose shell costs what a kit's does, a band of geometry per storey; `dressed` hangs a kit's signs, screens and panes on the near building only; `whole` has no far look at all:

| | open | meshes | triangles held | at the spawn | draws | worst frame |
|---|---|---|---|---|---|---|
| kitted, shells everywhere | 766 ms | 37 | 595,380 | 431,868 | 31 | 3.92 ms |
| kitted, shells to 256 m | 414 ms | 36 | 101,880 | 54,900 | 31 | 10.73 ms |
| dressed, shells everywhere | 456 ms | 35 | 72,084 | 27,036 | 29 | 2.14 ms |
| dressed, shells to 256 m | 384 ms | 34 | 43,992 | 27,156 | 29 | 3.69 ms |
| dressed, no far look | 524 ms | 26 | 126,348 | 72,096 | 23 | 10.31 ms |

A greybox shell is a box and so is a massing, so the greybox rows only prove that nothing was lost; the win is the size of what the far field would have been drawing, which is what `kitted` stands in for.

The same town in a headless Chromium on the Radeon 8060S at 1920 by 937, greyboxed, sixteen lights live, the render pass timed on the GPU (`BLOCKS=20 SHELL=all node game/scene/tools/bench/measure.ts street 16`, and `street-gl` for the WebGL2 backend):

| | open | draws, WebGL2 | draws, WebGPU | triangles | render pass, WebGL2 | render pass, WebGPU |
|---|---|---|---|---|---|---|
| shells everywhere | 466 ms | 29 | 15,599 | 310,227 | 1.79 ms | 1.66 ms |
| shells to 256 m | 434 ms | 30 | 15,625 | 310,539 | 1.87 ms | 1.68 ms |

Greyboxed the pass does not move, which is the point of the row: the skyline costs a draw and the same triangles a box did. `renderer.info.render.drawCalls` on the WebGPU backend counts one draw per visible instance of a batch, 15,625 for the same scene the WebGL2 backend multi-draws in 30, which is that backend's way of submitting a `BatchedMesh` and is the same however the town is built.
...
| | cells | plots | pieces | meshes | triangles |
|---|---|---|---|---|---|
| 1 block, swept | 41 x 39 | 8 | 0 | 23 | 7,078 |
| 1 block | | | 408 | 24 | 9,988 |
| 5 blocks, swept | 167 x 161 | 227 | 0 | 32 | 36,492 |
| 5 blocks | | | 5,559 | 33 | 82,088 |
| 7 blocks, swept | 214 x 213 | 395 | 0 | 33 | 51,324 |
| 7 blocks | | | 9,745 | 34 | 132,328 |

Rubbish is about 8 triangles a piece and it scales with paved area rather than with plots, which is what moved the triangle ceiling from 60,000 to 70,000: a 4 m pavement is twice the ground a 2 m one was and it carries a band against the building line as well as one in the gutter. A flat `sack` is six of those triangles, because a pavement carries hundreds of them.

The paint is two instanced meshes at any size: 120 rectangles on the one block town, 1,108 on five blocks and 1,958 on seven, of which the broken lane lines are 80 and 102. A four lane road is a handful more instances in the same two draws.
...
**What walking through a door costs.** `city.interior` is a demand call: the player is at the door and the room has to be there, so it is not budgeted the way the streaming is. Measured on the metro 20 by 20 city over its first 24 doors, the room dressed by `@gb/kitbash`: 2.5 to 4.7 ms median, 9 to 19 ms at the worst door, of which the dressing's own props, bodies and pickups are 0.3 to 3 ms and the rest is this box's shell, its blockers and its visitor cells. That is inside one frame's budget, so a room is still built whole. A dressing whose props cost more than that is what would make it worth streaming, and the number to watch is the one the dressing spends.
...
city.follow(player.x, player.z, dt) // the lights to the nearest emitters, the shells, the detail and the rooms to the player's cell
```

Without them the street is a dark, grimy, dry road that reflects the neon and lights the spawn: right for the hours the game is mostly played in, wrong at noon. Give `follow` the frame's own elapsed time and it streams the buildings in a few at a time; give it none and it builds the lot on that call, which is what opening a city wants and what a ride between stations wants behind its veil. Indoors, `follow` is given the door the player came in by, so the street outside stays dressed and the room stays built.
...
A real art kit is a new `Dressing`, not a change here. A dressing that wants its buildings batched has only to return indexed meshes on shared materials; welding a building's own pieces per material first, the way `@gb/kitbash` does, keeps the batch's instance count down but is not required. A kit that wants its far buildings cheap implements `shell(plot, size, charter)`: the walls and roof on the same footprint, with nothing on them that is only read from the pavement, and everything else stays in `building`. What a dressing answered, read so that nothing it does can empty the street, is `src/seam.ts` alone. How far each ring reaches is `DETAIL_RADIUS` and `SHELL_RADIUS` in `src/lod/near.ts` alone, and which buildings are in one; how a ring comes and goes with the player, and how much of its queue a frame takes, is `src/lod/ring.ts` alone, and what a frame may spend on it `src/lod/budget.ts`; the skyline every plot stands in whatever else it is wearing is `src/lod/massing.ts` alone, and which of the three a building is drawn at `src/batch/building.ts`; how rooms are kept and let go is `src/lod/rooms.ts`. How a batch is cut, how it grows and what growing costs the renderer is `src/batch/batch.ts` alone; what a camera would really draw, which is how the streaming is judged, is `tests/seen.ts`. Where a visitor may stand is `src/visitors.ts` alone. A kit that wants worn road paint implements `marking(paint)`, one that wants its own rubbish material implements `clutter()`, and one whose buildings should light the street implements `lights(plot, size, charter)`; leave any of them out and the street gets a plain one, or no light. How many emitters are live and how they are picked is `src/lights/city-lights.ts` alone; what a room is lit by is `src/lights/room-lights.ts` alone, and how much of that comes back off its own surfaces `BOUNCE` in `src/fill.ts`; the shell of a room and its metre UVs are `src/shell.ts` and `src/metre-uv.ts`. What the ground is made of is the dressing's (`ground(kind)`); what the street has been through is this box's, and the two compose, so a pale kit surface will read pale through the film however dark the film is. Anything that needs the renderer, the camera, input or a frame loop belongs in the app, not in this box: everything here builds objects and returns them, which is why it is tested in Node with no browser. Where a thing left in a room lands is `src/surface.ts` and `src/leaving.ts` alone: the first measures the top of a piece off its triangles rather than off a table, so a kit that redraws a counter needs nothing here, and the second is the one rule both the build and `leave` go through. How a room's things are drawn together is `src/pickups.ts` alone. Retuning how much rubbish a street carries is `CLUTTER_DENSITY` alone; retuning what a piece of rubbish looks like is `src/clutter/models.ts` alone, and how big it is `src/clutter/catalog.ts`, which the placement reads; the small painted geometry both the rubbish and the skyline are put together from is `src/assembly.ts`. How big a piece of surface detail is in the real world is `src/street/sizes.ts` alone. The towns everything here is measured on are `tests/town.ts`: `Forge.plan` lays the streets, the roads and the buildings, which takes no narrator and answers at once, and `tests/rooms.ts` cuts a few rooms in behind their doors, because a plan opens none. The benches under `tools/bench/` stand up the same town. Run `pnpm --filter @gb/scene test`.
