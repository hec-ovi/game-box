...
import { waterLevelAt } from './water.ts'
import type { Basin } from './height.ts'
...
const WOOD_SCALE = 190
/** Metres of the band around town sown at the theme's own spacing; past it, a third as thick. */
const NEAR = 480
...
export function buildTrees(
  ...
  const spots = plant(world, ground, height, basins, theme, noise, rng, budget)
...
function plant(
  ...
  const { spacing, reach } = theme.trees
  const found: Spot[] = []
  // close in at the theme's own spacing, then thinner over the kilometres past it
  sow(found, world, ground, height, basins, theme, noise, rng, { reach: NEAR, step: spacing, skip: 0 })
  sow(found, world, ground, height, basins, theme, noise, rng, { reach, step: spacing * 3, skip: NEAR })
...
/** Walk one band of ground on a jittered lattice and keep what can hold a tree. */
function sow(
  into: Spot[],
  world: World,
  ground: Ground,
  ...
  noise: Noise,
  rng: Rng,
  band: { reach: number; step: number; skip: number },
): void {
  const { density, treeLine, maxSlope, species } = theme.trees
  ...
  for (let z = -band.reach; z < bottom + band.reach; z += band.step) {
    for (let x = -band.reach; x < right + band.reach; x += band.step) {
      ...
      const px = x + (rng.float() - 0.5) * band.step
      const pz = z + (rng.float() - 0.5) * band.step
      const turn = rng.float() * Math.PI * 2
      const size = rng.range(0.75, 1.35)
      const stretch = rng.range(0.85, 1.2)

      if (band.skip > 0 && px > -band.skip && px < right + band.skip && pz > -band.skip && pz < bottom + band.skip) {
        continue
      }
      // inside the map, only the verge belongs to the land: the rest is town
      ...
      if (waterLevelAt(basins, ground, px, pz) !== undefined) continue
      if (ground.slopeAt(px, pz) > maxSlope) continue
      if (wooded(noise, px, pz) < 1 - density) continue

      into.push({ x: px, y, z: pz, species: rng.weighted(shares), turn, size, stretch })
    }
  }
}
...
function treeGeometry(species: Species): THREE.BufferGeometry {
  ...
  const trunk = new THREE.CylinderGeometry(trunkWidth * 0.7, trunkWidth, trunkHeight, 4, 1, species.shape !== 'bare')
  trunk.translate(0, trunkHeight / 2, 0)
  parts.push(tint(trunk, species.trunk))

  if (species.shape === 'cone') {
    const canopyHeight = species.height - trunkHeight * 0.4
    const cone = new THREE.ConeGeometry(species.spread / 2, canopyHeight, 6, 1)
    ...
    for (const [lift, lean] of [[0.62, 1], [0.8, -1]] as const) {
      const branch = new THREE.CylinderGeometry(trunkWidth * 0.3, trunkWidth * 0.45, species.spread * 0.7, 3, 1, true)
