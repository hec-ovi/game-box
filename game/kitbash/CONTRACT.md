...
- The art: Quaternius Downtown City MegaKit, CC0, packed by `tools/build-kit.ts` into `assets/dist/downtown-kit.glb` (19 wall pieces, 3,403 triangles and 3 ground surfaces, 2.07 MB). The street lamps are not art: they are cut from primitives here. The pack is meshopt-compressed and quantized, and its dedup step folds the kit's 10 material names into the 6 that have distinct textures. The roadway is painted by the kit's own asphalt, so it costs nothing over the buildings: it is one copy shared with the kit's own road piece.
- The ground tiles: the pavement and the earth are generated images, cut seamless by `tools/textures/tile.mjs`, and committed under `assets/gen` with the normal and the wear derived from each by `tools/textures/relief.mjs`. They are ours, so they ship inside a world file like anything else generated here.
...
- **The glass claims its wall first, then the signs.** Every glazed module above the street takes the patch it covers before any signage is offered a place, so nothing is hung over a window somebody lives behind. The street is left alone on purpose: a shopfront glazes right round the corner and its name belongs on the fascia band over that glass, and the door lamps and the camera live down there too. `glazingFor` publishes those patches so the promise can be checked from outside. Then every panel takes the patch it stands on, with 12 cm of air round it, and a later one landing on a held patch is not drawn; a hung box claims the width of its bracket. Each of the small lit things offers both ends of its wall and the first free one is taken. Measured on a town of 160 plots and 785 signs: no two of them overlap, where before the claim 96 pairs did.
...
- The ground is toned by the same theme. The kit's asphalt is 0.042 linear and the paving flag is 0.251, so the roadway already sits where `@gb/scene`'s wet film paints it and the pavement is the one that reads pale after dark: with the walls at 0.01 and nothing but lamps and signs lighting them, a pale pavement is the brightest thing in the street. `PAVEMENT_TONES` takes it to 0.091 in a neon town and 0.111 in an industrial one, and leaves the other five at the whole flag, because a pale pavement under a sun is what a pavement looks like. The park and the bare lot are tinted down off the same earth to 0.066 and 0.082, so nothing unbuilt out-shines the pavement. The kerbs go with it: they are the same material.
...
- The ground tiles at a real-world size, on every map it has. `@gb/scene` lays ground UVs out in metres, so a surface that should repeat every `tile` metres is set to `repeat = 1 / tile`: all three every 4 m, which lays the pavement in 0.67 m flags whatever the cell size is. Colour, relief, roughness and occlusion all take the same repeat off the same tile, so nothing on a surface slides against anything else on it.
...
- Colour comes from the tint over a shared texture, not from a texture each: the same earth is a park greened down and a bare lot greyed down. A town's tone scales that tint rather than replacing it, so a dark street still has flags, joints and wear on it.
...
- The road borrows the kit's asphalt without changing it: the maps come off the pack cloned, so tiling the road at 4 m leaves the kit's own road piece painted the way it was authored.
- **The relief and the wear are ours, and they had to be.** A generated colour tile carries no maps at all, and the ORM the kit ships with its asphalt has an occlusion channel that is a flat 255, so there is no hollow in either to shade. `tools/textures/relief.mjs` reads each ground surface's own colour map, subtracts the wavelengths longer than the family's cut (stain, not shape), and writes the shape into a normal map and the hollow and the roughness into red and green of a wear image. The roughness is the material we are claiming rather than the one the image happened to expose at: a concrete flag ships at 0.74 to 0.98 and bare earth at 0.90 to 0.99, both off `tools/textures/relief/surfaces.mjs`.
...
Measured off the shipped pack. Colour and relief are 1024 px; the wear is 512, because a roughness and a hollow are low frequency next to a colour map and half the resolution is a quarter of the memory.
...
| paving | 4 m | 3.91 mm | 4.65 / 22.77 deg | 0.74 to 0.98 | 0.45 |
| earth | 4 m | 3.91 mm | 27.61 / 58.38 deg | 0.90 to 0.99 | 0.69 |

Nineteen images, and the pack is 2.07 MB. The pavement and the earth carry a generated colour tile and a normal derived from it, where they used to carry the kit's marble and dirt: the same four 1024 maps, so what is resident is unchanged and only the compressed bytes moved, because there is more in the new pictures. Every surface also carries one 512 wear image. Nothing new is drawn and no material is added: the ground is the same six materials it always was.
